...
let
  ...
  windows_dev_env = pkgs.writeText "windows-dev.env" ''
    # Generated by Home Manager. This value is not secret.
    ${
      if cfg.windows_dev.workspace_root == null then
        ""
      else
        "WINDOWS_DEV_WORKSPACE_ROOT=${lib.escapeShellArg cfg.windows_dev.workspace_root}"
    }
  '';
  ...
  windows_dev_mcp = pkgs.writeTextFile {
    ...
    executable = true;
    destination = "/bin/windows-dev-mcp";
  };
...
in
{
  options.programs.contextforge = {
    ...
    windows_dev.workspace_root = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = null;
      description = "Absolute Windows or WSL path to the allowed windows-dev workspace root.";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile."contextforge/windows-dev.env" = {
      source = windows_dev_env;
      force = true;
    };
