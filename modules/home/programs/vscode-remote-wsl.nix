...
let
  ...
  baselineFile = pkgs.writeText "vscode-remote-wsl-settings.json" (builtins.toJSON baselineSettings);
  boolStr = b: if b then "true" else "false";
in
{
  options.programs.remoteVscode = {
    enable = lib.mkEnableOption "Manage Windows VS Code (Remote WSL) settings.json font-related keys";
    backup = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Create timestamped backup of existing settings.json before modifying.";
    };
    enforce = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "If true, override font keys every activation. If false, only add them when missing.";
    };
    syncFonts = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "If true, copy managed font files into Windows per-user Fonts directory and register them.";
    };
    powerShellCommand = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = null;
      description = "Override command/path to invoke Windows PowerShell (powershell.exe) or PowerShell 7 (pwsh.exe). If null, attempt auto-detection.";
    };
    fontWhitelistRegex = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = "(Fira|Nerd|Deja|Source|JetBrains)";
      description = "Case-insensitive regex applied to font filenames; only matches are synced. Null disables filtering.";
    };
    addRunKey = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "If true, install an HKCU Run key that re-registers all synced fonts at each Windows logon (persistence across reboot).";
    };
  };

  config = lib.mkIf cfg.enable {
    home.packages = [ pkgs.jq ];
    home.activation.vscodeRemoteSettings = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      set -eu
      ...
      if [ "${boolStr cfg.syncFonts}" = true ]; then
        psCmd="${lib.optionalString (cfg.powerShellCommand != null) cfg.powerShellCommand}"
        if [ -z "$psCmd" ]; then
...
PS_BODY
      ...
      if [ "${boolStr cfg.syncFonts}" = true ] && [ "${boolStr cfg.addRunKey}" = true ]; then
        if [ -n "$psCmd" ]; then
